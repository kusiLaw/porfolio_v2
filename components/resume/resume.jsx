...
const Resume = ({isOpen}) => {
  return (
   <Link href={'https://docs.google.com/document/d/1pTXjshj-gHeQ-aqIyUDYjvYG_WbaWTPRaA-69UEuj1Y/edit?usp=sharing'} className={isOpen ? 'inline-block border-2 border-dotted  mt-5 px-3 py-3  \
   ...
   hover:text-white hover:border-bg-light_accent  hover:bg-light_accent  \
    hover:dark:text-white hover:border-bg-dark_accent hover:dark:bg-dark_accent'} target="blank">{isOpen ? "Get My Resume" : "Resume"}</Link>
  )
}
